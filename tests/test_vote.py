...
def test_vote_on_post(authorized_client, test_post):
    res = authorized_client.post('/vote/', json={'post_id':test_post[0].id, 'dir':True})
...
def test_vote_post_non_exist(authorized_client, test_post):
    res = authorized_client.post('/vote/', json={'post_id':6546546546, 'dir':True})
    assert res.status_code == 404


def test_vote_unauthorized_user(client, test_post):
    res = client.post('/vote/', json={'post_id':test_post[0].id, 'dir':True})
